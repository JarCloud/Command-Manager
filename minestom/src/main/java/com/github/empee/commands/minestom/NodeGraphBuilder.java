...
public class NodeGraphBuilder {
  ...
  @Getter
  private final PropertiesMapper PROPERTIES_MAPPER = new PropertiesMapper();
  ...
  private Node buildArgumentNode(Argument<?> argument) {
    ...
    if (hasProperties) {
      node.properties = PROPERTIES_MAPPER.mapToByteArray(argument.getProperties());
    }
  ...
  public List<Node> buildNodeGraphOf(CommandNode<?> command, Player source) {
    ...
    for (CommandNode alias : command.getAliases()) {
      nodes.addAll(buildNodeGraphOf(alias, source));
    }
    ...
    for (CommandNode child : command.getChildren()) {
      ...
      if (child.getAccessTester() != null && child.getAccessTester().apply(source) != null) {
        continue; //Ignore this node, player doesn't have access to it
      }

      var childNodes = buildNodeGraphOf(child, source);
